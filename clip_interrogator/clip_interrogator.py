...
from transformers import AutoProcessor, LlamaTokenizer, Blip2Processor, Kosmos2Processor, AutoModelForVision2Seq, \
...
CAPTION_MODELS = {
    ...
    'blip2-flan-t5-xxl': 'Salesforce/blip2-flan-t5-xxl',         #~19.5 GB 
    'git-large-coco': 'microsoft/git-large-coco',                # 1.58GB
    'blip2-opt-6.7b': 'Salesforce/blip2-opt-6.7b',               #~9.5 GB 
    'blip2-opt-6.7b-coco': 'Salesforce/blip2-opt-6.7b-coco',     #~9.5 GB 
...
class Config:
    ...
    cache_model_path: str = None # path to store downloaded models
    download_models_to_cache: bool = False # when true, entire model are downloaded from huggingface to cache_model_path
    download_cache: bool = True # when true, cached embeds are downloaded from huggingface
    cache_path: str = './cache/embeddings'   # path to store cached text embeddings
...
class Interrogator():
    ...
    def download_model_from_HF(self):
        #print(self.config.caption_model_name)
        self.config.model_path = CAPTION_MODELS[self.config.caption_model_name]        
        #print(self.config.cache_model_path)
        #print(model_path)
        
        if self.config.download_models_to_cache:
            cached_model_path = os.path.join(self.config.cache_model_path, self.config.model_path)
                
        if self.config.download_models_to_cache and not os.path.exists(cached_model_path):
            from huggingface_hub import snapshot_download
            try:                    
                snapshot_download(repo_id=self.config.model_path, local_dir=cached_model_path, local_dir_use_symlinks=False)                
            except Exception as e:
                print(f"Failed to download model: {self.config.model_path} from HF")
                print(e)
                return False
                
        self.config.model_path = cached_model_path if self.config.download_models_to_cache and os.path.exists(cached_model_path) else self.config.model_path         
    
    def load_llava_caption_model(self):        
    ...
    def load_clip_caption_model(self):
        if self.config.caption_model is None and self.config.caption_model_name:
            ...
            if self.config.caption_model_name.startswith('git-'):
                caption_model = AutoModelForCausalLM.from_pretrained(self.config.model_path, low_cpu_mem_usage=True, local_files_only=self.config.download_models_to_cache, torch_dtype=self.dtype, **kwargs)
                ...
                caption_model = Blip2ForConditionalGeneration.from_pretrained(self.config.model_path, low_cpu_mem_usage=True, local_files_only=self.config.download_models_to_cache, torch_dtype=self.dtype, **kwargs)
                self.caption_processor = Blip2Processor.from_pretrained(self.config.model_path)
            elif self.config.caption_model_name.startswith('kosmos-2'):
                caption_model = AutoModelForVision2Seq.from_pretrained(self.config.model_path, low_cpu_mem_usage=True, local_files_only=self.config.download_models_to_cache, torch_dtype=self.dtype, **kwargs)
    ...
    def load_caption_model(self):
        ...
        if self.config.caption_model_name.startswith('llava'):
            self.load_llava_caption_model()
        else:
            self.load_clip_caption_model()
    ...
    def generate_caption(self, question_prompt, caption_max_length, pil_image: Image) -> str:
        assert self.caption_model is not None, "No caption model loaded."
        self._prepare_caption()        
        
        if self.config.caption_model_name.startswith('kosmos-2'):
            """
            Copied and adapted from https://github.com/lucataco/cog-kosmos-2/blob/main/predict.py
            """
            _prompt = f"<grounding>{question_prompt}"
            inputs = self.caption_processor(text=_prompt, images=pil_image, return_tensors="pt").to(self.device)
            ...
            generated_text = self.caption_processor.batch_decode(tokens, skip_special_tokens=True)[0]
            ...
            return captions
        elif self.config.model_path or self.config.caption_model_name.startswith('cogagent'):
            ...
            inputs = {
                ...
                'images': [[input_by_model['images'][0].to(self.device).to(self.dtype)]] if pil_image is not None else None,
            }
            ...
            tokens = self.caption_model.generate(**inputs, 
                                                 max_new_tokens=caption_max_length)
            captions = self.caption_processor.batch_decode(tokens, skip_special_tokens=True)[0].strip()            
        return captions
    ...
    def interrogate_classic(self, image: Image, question_prompt:str='', caption_max_length: int = 2048, max_flavors: int=3, caption: Optional[str]=None) -> str:
        """Classic mode creates a prompt in a standard format first describing the image, 
        then listing the artist, trending, movement, and flavor text modifiers."""
        caption = caption or self.generate_caption(question_prompt, caption_max_length, image)
        caption = _remove_last_char(caption)
        if(self.config.generate_features):
    ...
    def interrogate_fast(self, image: Image, question_prompt:str='', caption_max_length: int = 2048, max_flavors: int=32, caption: Optional[str]=None) -> str:
    ...
    def interrogate(self, image: Image, question_prompt:str='', caption_max_length: int = 2048, min_flavors: int=8, max_flavors: int=32, caption: Optional[str]=None) -> str:
    ...
    def _prepare_caption(self):
        if self.config.clip_offload and not self.clip_offloaded:
            ...
            self.clip_offloaded = True
        if self.caption_offloaded and not (self.config.load_4bit or self.config.load_8bit):
    ...
    def _prepare_clip(self):
        if self.config.caption_offload and not self.caption_offloaded:
            ...
            self.caption_offloaded = True
        if self.clip_offloaded and not (self.config.load_4bit or self.config.load_8bit):
...
def _remove_last_char(text:str) -> str:
...
def list_caption_models() -> List[str]:
    caption_models = list(CAPTION_MODELS.keys())
    return sorted(sorted(caption_models, key=lambda t: t[1]), key=lambda t: t[0], reverse=False)


def list_clip_models() -> List[str]:
    clip_models = list(['/'.join(x) for x in open_clip.list_pretrained()])
    to_be_removed = {'convnext_xxlarge/laion2b_s34b_b82k_augreg',
    ...
    clip_models = [item for item in clip_models if item not in to_be_removed ]
    return sorted(sorted(clip_models, key=lambda t: t[1]), key=lambda t: t[0], reverse=False)
